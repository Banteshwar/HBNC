...
//        parent::__construct();
    ...
    public function getUser($memeberId = 0) {
        ...
        $stmt = $this->_db->prepare('SELECT * FROM ' . TB_T_MEMBERS . ' WHERE memberID = :memberID');
    ...
    public function get_user_by_Name($username) {
        ...
        try {
            ...
            $stmt = $this->_db->prepare('SELECT * FROM ' . TB_T_MEMBERS . ' WHERE username = :username');
...
//        showData($this->_db);
        ...
        if ($this->password_verify($password, $hashed) == 1) {
            ...
            if (checkLogin($user_row['memberID'])) {
                $stmt = $this->_db->prepare('UPDATE ' . TB_T_MEMBERS . ' set LastLogin = NOW() WHERE username = :username');
                $stmt->execute(array('username' => $username));
    ...
    public function isUsernameExist($username, $excludeMemberId = 0) {
        if ($excludeMemberId != 0) {
            $stmt = $this->_db->prepare('SELECT * FROM ' . TB_T_MEMBERS . ' WHERE '
    ...
    public function isUserEmailExist($email, $excludeMemberId = 0) {
        if ($excludeMemberId != 0) {
            ...
            $stmt = $this->_db->prepare('SELECT * FROM ' . TB_T_MEMBERS . ' WHERE email = :email');
